...
static size_t  FindSentEndAfterParagraph (const CGraphmatFile &F, size_t StartPos, size_t EndPos)
{
	 ...
	 // ignore all "long delimiters" between paragraphs
     for (; (k > 0) && (F.HasDescr(k, OLongDelimSeq)|| F.GetUnits()[k].IsSpace()); k--);
...
/*
	this function sets graphematical desriptors to the end of the sentence.
...
{	
	...
	{
		F.SetDes(NewSentStart, OSentStart);
	};
		...
		)
	{
		...
		//  we calcultaing sentence offset from the previous sentence end, since 
		//  everybody uses OSentEnd as text delimiter. Nobody uses OSentStart
...
bool IsLastInGroupOrFree(const CGraphmatFile& F, size_t i) 
{
	...
	return 
			...
			|| F.HasDescr (i, ORef2)
			|| F.HasDescr (i, ODate2)
			|| F.HasDescr (i, OFloat2)
			|| F.HasDescr (i, OElectAddr2)
...
bool CGraphmatFile::DealSentBreaker ()
{
	...
	*/
	...
	for (size_t WordNo = 1; WordNo < EndPos; WordNo++)
	{
		...
		{
			size_t StartSent = PPunct(WordNo, EndPos);
			...
			if (		InnerOpenBracket != 0 )
			{
				...
				*/
				size_t i = PSpace(WordNo+1, EndPos);
				if	(	( i<EndPos)
			...
			if (bSentenceQuotationMarks)
			{
				...
				size_t i = PSpace(SentEnd+1, EndPos);
				if	(		( i<EndPos)
		...
		if	(		!GetUnits()[WordNo].IsSoft()
				&&	(GetTokenInputOffset(WordNo) > StartSentenceOffset + 9000)
			)
		{
			// Sent1 and Sent2  cannot be at the same graphematical line 
			size_t StartSent = PSoft(WordNo+1, EndPos);
