...
static string ConvertASCIIToHtmlSymbols(const string& txt)
{
	string out;
	char asciiCode[2];
	asciiCode[1] = 0;
...
string  CConSent :: GetString() const
{
	...
	{
		char s[MaxGraLen+1];
		strncpy (s, m_GraFile->GetUnits()[i].GetToken(), m_GraFile->GetUnits()[i].GetTokenLength());
		s[m_GraFile->GetUnits()[i].GetTokenLength()] = 0;
