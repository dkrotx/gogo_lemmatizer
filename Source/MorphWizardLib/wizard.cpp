...
//==============================================================================
bool CFlexiaModel::ReadFromString(const string& s)
{
...
bool MorphoWizard::load_wizard(const char *path,const char *user_name, bool bCreatePrediction)
{
	...
	char msg[128];
	sprintf(msg,"Opened by %s",m_Sessions.back().m_UserName.c_str());
	log(msg);
...
void ReadFlexiaModels(FILE* fp, vector<CFlexiaModel>& FlexiaModels )
{
	...
	for(int num = 0; num < paradigm_count; num++)
	{
		...
		if (!M.ReadFromString(line))
			throw CExpc(Format("Cannot parse paradigm No %i", num+1));
...
static void ReadSessions (FILE* fp, MorphoWizard& W)
{
	W.m_Sessions.clear();
...
void MorphoWizard::ReadPrefixSets (FILE* fp)
{
	...
	for(int b = 0; b < count; b++)
	{
		...
		if (PrefixSet.empty())
			throw CExpc(Format ("No prefixes found in prefix sets section"));
...
void MorphoWizard::save_mrd()
{
	...
	m_Sessions.back().m_LastSessionSave = GetCurrentDate();
	...
	for(lemma_iterator_t b = m_LemmaToParadigm.begin(); b != m_LemmaToParadigm.end(); ++b)
	{
		...
		if(base.empty()) base = "#";
		string s1 = (b->second.m_CommonAncode[0] == 0) ?  "-" :  b->second.GetCommonAncode();
	...
	log( Format("Saved by %s",m_Sessions.back().m_UserName.c_str()) );
};
...
void MorphoWizard::find_lemm(string lemm, bool bCheckLemmaPrefix, vector<lemma_iterator_t> &res) 
{
	...
	if( (lemm.length() > 2) &&	(lemm[0] == '/') &&	(lemm[lemm.length()-1] == '/') )
	{
		...
		pcre *re = compile_prce(pattern.c_str());;
		if (re == NULL)
			return;

		for( lemma_iterator_t it=m_LemmaToParadigm.begin(); it!=m_LemmaToParadigm.end(); it++ )
		{
			if (has_regular_expression(re,it->first.c_str(),it->first.length()) )
				res.push_back(it);
...
void MorphoWizard::find_wordforms(string wordform, vector<lemma_iterator_t> &res)
{
	...
	if (		(wordform[0] != '/') 
		...
		for (lemma_iterator_t it = m_LemmaToParadigm.begin(); it != m_LemmaToParadigm.end();it++)
		{
			...
			{	
					if (has_regular_expression(re, wordforms[i].c_str(),wordforms[i].length()) )
					{
						res.push_back(it);
						break;
					}
...
{
	string s = it->second.GetCommonAncode();
	if (s.empty()) return "";
...
string MorphoWizard::get_slf_string(lemma_iterator_t it, string &common_grammems, string& Prefixes,  int line_size)
{
	...
	common_grammems = get_grammem_string(I.GetCommonAncode());
...
string MorphoWizard::mrd_to_slf(const string &lemm, const CFlexiaModel &p, WORD AccentModelNo, BYTE AuxAccent, int line_size) const
{
	...
	for(size_t n = 0; n<p.m_Flexia.size(); n++)
	{
		...
		for(int i = 0; i < code.size(); i+=2)
		{
			...
			string grammems = get_pos_string(gramcode) + " " + get_grammem_string(gramcode);
...
/*
BYTE FindLastUpperVowel(const string& s, MorphLanguageEnum Language, int& IndexFromStart)
{
	int CountOfVowels = 0;

	for (IndexFromStart=s.length()-1; IndexFromStart >=0; IndexFromStart--)
	{
		if (is_lower_vowel((BYTE)s[IndexFromStart], Language) )
			CountOfVowels++;

		if (is_upper_vowel((BYTE)s[IndexFromStart], Language))
...
BYTE FindFirstUpperVowel(const string& s, MorphLanguageEnum Language)
{
	for (size_t i=0; i <  s.length(); i++)
		if	(is_upper_vowel((BYTE)s[i], Language) )
			return i;
...
/* // старый вариант без сортировки по ударениям (закомментировано Кецарисом) [12/Apr/2004]
void MorphoWizard::slf_to_mrd(const string &s, string &lemm, CFlexiaModel& FlexiaModel, CAccentModel& AccentModel, BYTE&  AuxAccent, int& line_no_err) const
{
	...
	typedef multimap<string, CSlfLineByAncode, AncodeLess > Gramcode2SlfLineMap;
	...
	for(size_t LinesCount = 1; lines(); LinesCount++)
	{
		line_no_err = LinesCount;
		string form, pos_and_grammems;
		if (TestForFullDublicates.find(lines.val()) == TestForFullDublicates.end() )
			TestForFullDublicates.insert(lines.val());
		else
			continue;

		StringTokenizer tok(lines.val(), "\t ");
		if(!tok())	throw CExpc(Format("error at line %i", LinesCount));
		form = tok.val();
		if(!tok())	throw CExpc(Format("error at line %i", LinesCount));
		pos_and_grammems = tok.val();
		if(tok())  pos_and_grammems += string(" ") + tok.val();
		if(form.empty() || pos_and_grammems.empty() || tok())
			throw CExpc(Format("error at line %i", LinesCount));
		
		string gramcode;
		if( !slf2ancode(pos_and_grammems, gramcode) )
			throw CExpc(Format("Wrong grammem at line %i (%s)", LinesCount, pos_and_grammems.c_str()) );
		...
		for (int k=form.length()-1; k >= 0; k--)
		{
			...
			if (form[k] == '\'') 
			{
				CountOfAccents++;
				if (CountOfAccents > 2)
					throw CExpc(Format("Too many stresses at line %i", LinesCount) );
				if (		(k == 0) 
						||	!is_lower_vowel((BYTE)form[k-1], m_Language)
					)
					throw CExpc(Format("A stress should be put on a vowel (line %i)", LinesCount) );
				...
				else
				{
					...
					{
						if (AuxAccent != k-1)
							throw CExpc(Format("Auxiliary stress should be fixed (line %i", LinesCount) );
					}
					else
						AuxAccent = k-1;
				};
		...
		else
		{
			...
			Paradigm.insert(make_pair(gramcode, Line) );
	...
	for(Gramcode2SlfLineMap::const_iterator it =  Paradigm.begin(); it != Paradigm.end(); it++)
	{
		string Flexia = it->second.m_Form.substr(CommonLeftPartSize);
		string Gramcode = it->first;
		BYTE AccentByte = it->second.m_AccentByte;
		string Prefix = it->second.m_Prefix;
...
{
	if( m_Sessions.back().m_UserName == "guest" )
		return;
...
//----------------------------------------------------------------------------
CParadigmInfo MorphoWizard::add_lemma(const string &slf, string common_grammems, const string& prefixes, int& line_no_err)
{
	string lemm;
	...
	CParadigmInfo NewInfo(ParadigmNo, AccentModelNo, GetCurrentSessionNo(), AuxAccent, common_gramcode.c_str() , PrefixSetNo);
...
inline string GetSuffix(const string& Lemma)
{
	int SuffLen = Lemma.length() - 2;
...
{
	m_PredictIndex.clear();
	lemma_iterator_t lemm_it = m_LemmaToParadigm.begin();
	...
	// going through all paradigms
	for (; lemm_it!=m_LemmaToParadigm.end(); lemm_it++ )
	{
		...
		S.m_Suffix = GetSuffix(lemma);
		S.m_SourceLemmaAncode = p.get_first_code();
		S.m_SourceCommonAncode = lemm_it->second.GetCommonAncode();
		S.m_SourceLemma = lemma;
		S.m_PrefixSetStr = get_prefix_set(lemm_it);

		S.m_Frequence = 1;
		if (S.m_SourceLemma.length() < 3) continue;
		vector<CPredictSuffix>::iterator it	= lower_bound (m_PredictIndex.begin(),m_PredictIndex.end(), S);
		if (		(it == m_PredictIndex.end())
				||	!(*it == S)
			)
			m_PredictIndex.insert(it, S);
		else
		{
			it->m_Frequence++;
			if (it->m_SourceLemma.length() > S.m_SourceLemma.length())
				it->m_SourceLemma = S.m_SourceLemma;
		}
		if( !!m_pMeter ) 
			m_pMeter->AddPos();
	}
	int size = m_PredictIndex.size();

};


void MorphoWizard::predict_lemm(const string &lemm, int minimal_frequence, bool bOnlyMainPartOfSpeeches)
{
	...
	try{
		...
		string Suffix = GetSuffix(lemm);
		
		for (int i =0; i < m_PredictIndex.size(); i++)
		{
			const CPredictSuffix& S = m_PredictIndex[i];
			if(S.m_Suffix != Suffix) continue;
			if (m_PredictIndex[i].m_Frequence < minimal_frequence) 
				continue;
			...
			string line = pos + " ";
			line += get_grammem_string(S.m_SourceLemmaAncode) + " ";
			line += lemm + string("    ");
			line += S.m_SourceLemma + string(" ");
			line += Format(" %i", S.m_Frequence);
			if ( lemm.find("|") != string::npos)
				if (S.m_PrefixSetStr.empty())
					continue;

			m_CurrentPredictedParadigms.push_back(i);
		};
...
{
	const CPredictSuffix& S = m_PredictIndex[m_CurrentPredictedParadigms[PredictParadigmNo]];
	const CFlexiaModel& P = m_FlexiaModels[S.m_FlexiaModelNo];
		...
		WORD NewFlexiaModelNo, WORD newAccentModelNo, bool keepOldAccents )
{
	if( NewFlexiaModelNo>=m_FlexiaModels.size() 
		|| newAccentModelNo>=m_AccentModels.size() && newAccentModelNo!=UnknownAccentModelNo 
		|| NewFlexiaModelNo==I.m_FlexiaModelNo && newAccentModelNo==I.m_AccentModelNo )
		return false;
		...
		*/
		...
		for (size_t i=0; i<NewFlexia.size(); i++)
		{
			...
			for (; k<OldFlexia.size(); k++)
				if	(	
					(OldBase+OldFlexia[k].m_FlexiaStr  == NewWordForm) &&	
...
bool MorphoWizard::attach_form_prefixes_to_bases()
{
	...
	for (int ModelNo = 0; ModelNo < m_FlexiaModels.size(); ModelNo++)
	{
		...
		for (; k< m_FlexiaModels[ModelNo].m_Flexia.size(); k++)
			if (!m_FlexiaModels[ModelNo].m_Flexia[k].m_PrefixStr.empty())
				break;

		if (k== m_FlexiaModels[ModelNo].m_Flexia.size()) continue;

		for( lemma_iterator_t it=m_LemmaToParadigm.begin(); it!=m_LemmaToParadigm.end(); it++ )
			if (it->second.m_FlexiaModelNo == ModelNo)
			{
				bFound = true;
				...
				{
					fprintf (stderr, "cannot put away form prefixes in \"%s\"!\n", it->first.c_str()); 
					fprintf (stderr, "Stopping the process!\n"); 
					return false;
				};
	...
	for (int ModelNo = 0; ModelNo < m_FlexiaModels.size(); ModelNo++)
	{
		for (size_t k=0; k< m_FlexiaModels[ModelNo].m_Flexia.size(); k++)
			if (!m_FlexiaModels[ModelNo].m_Flexia[k].m_PrefixStr.empty())
			{
				fprintf (stderr, "some morphological schemes are still with prefixes !\n"); 
				fprintf (stderr, "We cannot go further!\n"); 
				return false;
			};
	};
