...
long CTempArticle::IntersectByFields(const CTempArticle *Article) const
{
...
inline bool IsPartOf (const vector<TCortege10>& L1,  const vector<TCortege10>& L2, bool UseWildCards, long EmptyDomItemNo, BYTE MaxNumDom)
{
...
void CTempArticle::ReadFromDictionary(WORD UnitNo, bool VisualOrder, bool ReadOnly)
{
   ...
   for (int i= m_pRoss->GetUnits()[UnitNo].m_StartCortegeNo; i <= m_pRoss->GetUnits()[UnitNo].m_LastCortegeNo; i++)
	  if (find(m_DisabledFields.begin(), 
		       m_DisabledFields.end(), 
			   CTextField (m_pRoss->GetCortegeFieldNo(i), 
			                     m_pRoss->GetCortegeLeafId(i),
								 m_pRoss->GetCortegeBracketLeafId(i))) != m_DisabledFields.end())
		  m_DisabledCorteges.push_back(GetRossCortege(i));
	  else
        if (VisualOrder)
...
bool CTempArticle::WriteToDictionary()
{
	...
	for (i = 0; i < m_DisabledCorteges.size(); i++)
		m_pRoss->_AddCortege(m_DisabledCorteges[i]);

	Units[m_UnitNo].m_LastCortegeNo = m_pRoss->_GetCortegesSize()-1;

	if  ( (GetCortegesSize()+m_DisabledCorteges.size()) == 0)
	{
...
bool CTempArticle::IsModified() const
{
	...
	long StartPos = m_pRoss->m_Units[m_UnitNo].m_StartCortegeNo;
	long EndPos   = m_pRoss->m_Units[m_UnitNo].m_LastCortegeNo;

	if (!m_pRoss->m_Units[m_UnitNo].HasEmptyArticle() )
      for (size_t i=StartPos; i <= EndPos; i++)
	  {
		    // disabled cortgeges не были видны   пользователю и он не мог их изменять
			int k = 0;
			for (; k < m_DisabledCorteges.size(); k++)
				if (m_DisabledCorteges[k].EqualCortege(GetRossCortege(i),m_pRoss->m_MaxNumDom))
					break;

			if (k == m_DisabledCorteges.size())
