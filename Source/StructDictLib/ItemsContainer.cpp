...
inline long   TItemContainer::GetItemNoByItemStr (const char*  ItemStr, const char* DomStr) const
{
...
long      TItemContainer::GetItemNoByItemStr (const char*  ItemStr, BYTE DomNo) const 
{
    ...
    else
	{
      ...
      if (     (U  == m_DomItems.end())
	    ...
	    return (long) (U-m_DomItems.begin());
	};
}
...
void   TItemContainer::UpdateConstDomens()
{
  for (size_t i=0; i < m_Domens.size(); i++)
  {
	  CDomen& D = m_Domens[i]; 
  	  ...
  	  if (i ==  EmptyDomNo)
		for (long k =D.m_StartDomItem; k < D.m_EndDomItem; k++)
...
bool   TItemContainer::BuildDomens (char* LastReadLine)
{
  ...
  if (!domens) return false;

	fscanf  (domens, "%u\r\n",&DomensCount);

// не превосходит  максимального индекса
	if (DomensCount >= ErrUChar) 
	{
		     fclose (domens);
			 return false;
	 };
