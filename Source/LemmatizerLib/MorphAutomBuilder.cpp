...
CMorphAutomatBuilder::CMorphAutomatBuilder(MorphLanguageEnum Language) :	CMorphAutomat(Language)
{
	m_pRoot = 0;
...
{
	if (!CheckABC(WordForm))
	{
		fprintf (stderr, "%s - bad ABC    \n", WordForm.c_str());
		return false;
	};

	if (WordForm.rfind(AnnotChar) == WordForm.length() - 1)
	{
