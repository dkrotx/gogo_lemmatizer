...
CPredictBase::CPredictBase(MorphLanguageEnum langua) :m_SuffixAutomat(langua)
{
...
void CPredictBase::FindRecursive(int NodeNo, string& curr_path, vector<CPredictTuple>& Infos) const
{
	...
	if (N.IsFinal())
	{
		int i = curr_path.find(AnnotChar);
		assert (i != string::npos);
		int j = curr_path.find(AnnotChar, i+1);
		assert (j != string::npos);
		int k = curr_path.find(AnnotChar, j+1);
