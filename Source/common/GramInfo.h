...
struct CGramInfo 
{
	...
	bool  EqualOrLess (const CGramInfo& _X) const
	{ 
		return 
			   (_X.m_Language  == m_Language ) 
			&& ( (m_PartOfSpeechMask &  _X.m_PartOfSpeechMask) > 0)
			&& ( (m_Grammems &  _X.m_Grammems) == _X.m_Grammems)
				;
	};

	bool operator ==  (const CGramInfo& _X) const
	{
		return		(m_PartOfSpeechMask == _X.m_PartOfSpeechMask)
				&&  (_X.m_Language  == m_Language ) 
				&&	(m_Grammems == _X.m_Grammems);
	};
	
	bool operator <  (const CGramInfo& _X) const
	{
		if (m_Language != _X.m_Language)
			return m_Language < _X.m_Language;

		if (m_PartOfSpeechMask != _X.m_PartOfSpeechMask)
			return m_PartOfSpeechMask < _X.m_PartOfSpeechMask;

		return m_Grammems < _X.m_Grammems;
	};

	void AddGramInfo (const CGramInfo& _X) 
	{
		m_PartOfSpeechMask |= _X.m_PartOfSpeechMask;
		m_Grammems |= _X.m_Grammems;
	};
...
struct CGramInfoAndGraphemUnitOffset
{
	...
	int			m_UnitOffset;
	bool operator < (const CGramInfoAndGraphemUnitOffset& _X) const
	{
		return m_UnitOffset< _X.m_UnitOffset;
	};
};
