...
struct CChunkInfo 
{
	...
	bool operator == (const CChunkInfo & _X) const 
	{
		return	 (m_ChunkType == _X.m_ChunkType) 
			&&	 (m_ChunkLength == _X.m_ChunkLength) ;
	}	
	bool operator < (const CChunkInfo & _X) const 
	{
		if (m_ChunkType == _X.m_ChunkType)
			return m_ChunkLength < _X.m_ChunkLength;
...
struct CChunkOccurrence
{
	...
	bool operator < (const CChunkOccurrence& _X) const
	{
		if (m_StartUnitOffset == _X.m_StartUnitOffset)
			return m_LastUnitOffset < _X.m_LastUnitOffset;

		return m_StartUnitOffset < _X.m_StartUnitOffset;
	};
};
